...
def compare_to_state(s,states):
    ...
    exc = [10,20,40,50,60,70,80,90,500,2000,2001,2002,2010,2011,2012]
    # Iterate through all years 
    for year in range(1998,2015): # This does 1998 to 2014
        # Empty dictionary to hold correlation coefficients
        cor = {}
        # Iterate through states
        for z in states:
            if z == s: # Skips the step in loop if the state is 's'
                continue
            state = get_state(z,year,excludes=exc,reference=90) # Gets information for arbitrary state 'z'
            s_info = get_state(s,year,excludes=exc,reference=90) # 's'-specific information
            ...
            filename = "{0}_correlations_{1}.json".format(year,s) # String for file name
            if not os.path.exists('data/correlations/years/{}'.format(s)):
                os.mkdir('data/correlations/years/{}'.format(s))
        with open('data/correlations/years/{0}/{1}'.format(s,filename),'w') as f: # Opens temporary instance of file
            f.write(json.dumps(cor,sort_keys=True)) # Writes data to file

if __name__ == "__main__":
    # Import data file. Change all Ds and Ts with 0.    
    data = pd.read_csv("data/employment_data.csv",dtype=str).replace(to_replace='(D)',value=0).replace(to_replace='(T)',value=0)
